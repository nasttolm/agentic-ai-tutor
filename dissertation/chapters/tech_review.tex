...
\midrule
\textbf{\nolinkurl{microsoft/Phi-4-mini-instruct}} & 4.2 & \textbf{29.41} & 73.78 & \textbf{32.58} & \textbf{0.83}\\
...
Combines low-rank adapters with 4-bit (or 8-bit) base-model quantization to reduce memory, enabling larger bases on smaller GPUs. Adds quantization/dequantization overhead that may affect speed and sometimes memory \parencite{FineTuni58:online, QLoRAEff12:online}. On a 12 GB GPU, the extra quantization and dequantization steps can slow down training and make the implementation more complex, while the memory savings are not strictly necessary for a 3.8-4.2B parameter model.
...
\begin{enumerate}
    ...
    The solution should be free to run for an academic MVP and self-hostable to avoid recurring API fees.
    \item \textbf{Institutional suitability}
    Components should be compatible with university deployment, including predictable licensing and the ability to run on-premise.
...
\textbf{Piper} is described as a fast, local neural text to speech system and its repository is MIT licensed. This directly supports the cost/deployability requirement because it is designed for local usage, and it supports institutional suitability because the license is explicitly stated as MIT. In addition, rhasspy/piper-voices provides voice files where individual voice model cards may explicitly specify dataset licensing. For example, one voice model card lists a dataset license of CC0. This provides a clear licensing signal at the voice level, which is relevant for university deployment. \parencite{piperLIC61:online, rhasspyp13:online, dedeDEth1:online}
...
Based on the stated properties in the official repositories and documentation, Piper was selected as the default TTS engine. Piper aligns with the cost and deployability requirement because it is explicitly described as a local neural TTS system. It aligns with institutional suitability because its repository license is explicitly MIT. In addition, Piper voices provide per-voice model cards where dataset licensing can be stated explicitly, enabling a university to verify the licensing of the exact voice artifact used. Finally, the system may implements TTS behind a provider interface so that Piper can be replaced by another toolkit if institutional policies require a different licensing or model choice.

\subsection{Talking-Head}

\textbf{SadTalker} is described as Learning Realistic 3D Motion Coefficients for Stylized Audio-Driven Single Image Talking Face Animation. This description directly matches the target workflow of generating a talking-head video from a single avatar image and an audio track. The project also includes a highlights section stating that "The license has been updated to Apache 2.0, and we've removed the non-commercial restriction." \parencite{GitHubOp96:online}
...
\subsection{Video Selection Rationale}
Based on the stated properties in official repositories, SadTalker was selected for the MVP video component. The choice aligns with institutional suitability because the repository highlights explicitly state an Apache 2.0 license update and the removal of a non-commercial restriction. In contrast, Wav2Lip explicitly limits usage to personal/research/non-commercial purposes. MuseTalk provides permissive statements for code and trained model usage, but also states a non-commercial restriction for test data, which introduces an additional artifact-level compliance check. In addition to licensing, the stated scope of SadTalker (audio-driven single image talking face animation) matches the intended tutor avatar workflow more directly than approaches described primarily as lip-syncing or speech-to-lip generation. Finally, the system implements video generation behind a provider interface so that the selected video generator can be replaced if institutional policies require a different component.
