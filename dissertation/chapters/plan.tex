...
A continuous integration and delivery pipeline was implemented using GitHub Actions and Argo CD. The GitHub Actions workflow is organised into four jobs: lint, build, playwright, and scan. The build job depends on lint, while the Trivy-based scan job depends on the successful completion of the image build. The playwright job runs separately as part of the same workflow to validate frontend behaviour through smoke tests.
...
The lint job runs on a standard Ubuntu runner without a GPU. It first checks the Python syntax of all backend modules using py\_compile, covering both the main service (config.py, rag.py, inference.py, main.py, schemas.py, logging\_config.py, tts.py). It then validates the Kubernetes manifests for structural correctness using kubeconform. This job runs on every push and every pull request, providing fast feedback before a Docker image is built.
...
The build job runs only on pushes to the main branch and only if the lint job succeeds. It checks out the repository including Git LFS objects (the LoRA adapter .safetensors files), authenticates to the GitHub Container Registry (ghcr.io), and builds and pushes the backend Docker image. Two tags are produced: a latest tag for the default branch and a commit-SHA tag (for example, ghcr.io/nasttolm/ai-tutor-backend:a3f1c2d) that uniquely identifies each build. Build-layer caching through the GitHub Actions cache is enabled to reduce image build time on repeated pushes.
